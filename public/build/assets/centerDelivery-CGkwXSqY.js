document.addEventListener("DOMContentLoaded",()=>{const r=document.getElementById("web"),l=document.getElementById("remarks"),c=document.getElementById("latest-records"),m=document.querySelector('meta[name="csrf-token"]').getAttribute("content");r&&r.focus(),r.addEventListener("keydown",function(n){if(n.key==="Enter"){n.preventDefault();const o=l.value.trim(),s=this.value.trim(),t=document.getElementById("smsg");if(!s)return;fetch("/centerDelivery",{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-TOKEN":document.querySelector('meta[name="csrf-token"]').getAttribute("content"),"X-Requested-With":"XMLHttpRequest"},body:JSON.stringify({web:s,rmks:o})}).then(async e=>{const a=e.headers.get("content-type");if(!a||!a.includes("application/json"))throw new Error("Permission denied");return e.json()}).then(e=>{e.success?(r.value="",d(e.latest),document.getElementById("count").textContent=e.count||"0",document.getElementById("smsg").textContent=e.smsg,t.classList.remove("text-red-600"),t.classList.add("text-green-600")):(document.getElementById("smsg").textContent=e.smsg||"Failed to save record.",r.value="",t.classList.remove("text-green-600"),t.classList.add("text-red-600")),document.getElementById("remarks").value=""}).catch(e=>{document.getElementById("smsg").textContent=e.message,r.value="",document.getElementById("remarks").value=""})}});function d(n){if(!n.length){c.innerHTML='<p class="text-gray-500">No recent records.</p>';return}let o=`
