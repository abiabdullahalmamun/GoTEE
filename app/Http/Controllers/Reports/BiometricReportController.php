...
use App\Exports\BiometricPrintExport;
...
class BiometricReportController extends Controller
{
        ...
        // Filters
            if ($request->filled('from_date') && $request->filled('to_date')) {
                ...
                if($role== 5 ||  $role== 1 ||  $role== 13){
                    $userIds = AppLog::select('created_by')->whereBetween('Date',[$request->from_date, $request->to_date])->where('stepId',11); 
                }
                else{
                    $userIds = AppLog::select('created_by')->whereBetween('Date',[$request->from_date, $request->to_date])->where('centerId',$cenId)->where('stepId',11); 
                }
                if($userIds){
             ...
             // dd($cenId) ; 
            return view('reports.biometric_report.index', compact('data', 'centers', 'cenId','role','users'));
    ...
    {    
        ...
        if ($request->filled('from_date') && $request->filled('to_date')) {
            ...
            return Excel::download(new BiometricPrintExport($request), 'biometric-report.xlsx');
        }
        else{
            return redirect()->route('biometric-report.index')->with('error', 'Please select from & to Date');
        }
