...
use App\Exports\AppReceiveExport;
...
class ReceiveReportController extends Controller
{
    ...
    public function index(Request $request): View 
    {
        ...
        // Filters
            ...
            return view('reports.receive_reports.app-receive', compact('data', 'centers', 'users','cenId','role', 'VisaType','stickerTypeList', 'paymentSummary','corrTotal'));
           
        } catch (Exception $e) {
             $mess = $e->getMessage(); 
             dd($mess) ; 
               return view('reports.receive_reports.app-receive', [
                    'data' => collect(),
                    'centers' => collect(),
                    'users' => collect(),
                    'cenId' => null,
    ...
    {
        if ($request->filled('from_date') && $request->filled('to_date')) {
            return Excel::download(new AppReceiveExport($request), 'app_receive_report.xlsx');
        }
        else{
