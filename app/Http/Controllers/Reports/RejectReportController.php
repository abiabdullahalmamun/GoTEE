...
use App\Exports\RejectPrintExport;
...
class RejectReportController extends Controller
{
    ...
    public function index(Request $request): View 
    {
        ...
        // Filters
            ...
            $data = $query->with(['center', 'user' ,'rejectReasons'  ])
                ->orderBy('id', 'desc')
                ->paginate(5000)
                ->appends($request->all());
            ...
            return view('reports.rejectReport.index', compact('data', 'centers', 'users','cenId','role'));
           
        } catch (Exception $e) {
             $mess = $e->getMessage(); 
             dd($mess) ; 
               return view('reports.rejectReport.index', [
                    'data' => collect(),
                    'centers' => collect(),
                    'users' => collect(),
                    'cenId' => null,
    ...
    {
        if ($request->filled('from_date') && $request->filled('to_date')) {
            return Excel::download(new RejectPrintExport($request), 'rejectReport.xlsx');
        }
        else{
            return redirect()->route('rejectReport.index')->with('error', 'Please select from & to Date');
        }
...
//summary_type
    ...
    public function destroy($id){
         // dd($id) ;
         $rejectDel = RejectLog::find($id); 
         $del = $rejectDel->delete() ; 
          if($del){
            $deLog2 = RejectLogReason::where('web_ref', $id)->delete() ; 
