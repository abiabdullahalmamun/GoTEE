...
use App\Exports\OverridePrintExport;

class OverrideReportController extends Controller
{
    ...
    public function index(Request $request): View 
    {
        ...
        // Filters
            ...
            if ($request->filled('type')) {
                $query->where('active', $request->type);
            }
            ...
            return view('reports.override_reports.index', compact('data', 'centers', 'users','cenId', 'role'));
    ...
    {
        if ($request->filled('from_date') && $request->filled('to_date')) {
            return Excel::download(new OverridePrintExport($request), 'override_report.xlsx');
        }
        else{
            return redirect()->route('override-Report.index')->with('error', 'Please select from & to Date');
        }
    ...
    public function exportPDF(Request $request)
    {
        ...
        if ($request->filled('from_date') && $request->filled('to_date')) {
           ...
           if ($request->filled('center')) {
            ...
            // dd($data) ; 
            $pdf = Pdf::loadView('reports.pdf.apt_override',compact('data','from','to','cnt','usr'))->setPaper('a4', 'landscape');
            return $pdf->download('apt_override.pdf');
