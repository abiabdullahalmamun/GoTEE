...
use App\Http\Requests\StoreForeignReceiveRequest;
...
class  FrpReceiveController extends Controller
{
    /**
    ...
    public function index(Request $request): View
    {
        ...
        try {
            ...
            if($cenId){
                $cnt = Counter::select('id','center_id','counter_id','loginstate','tokenno','autoMan','visaType','stickerType')->where('ip',$lastHopIp )->first() ; 
                // dd($cnt) ; 
                if($cnt){
                    // dd($cnt) ; 
                    if($cnt->center_id === $cenId ){
                        ...
                        // $tokenhistory = DisplayScroll::where('Date', date('Y-m-d'))
                        //         ->where('centerId', $cenId)
                        //         ->where('svc_no', $lastSvc)
                        //         ->where('counterNo', $cntNo)
                        //         ->orderBy('id', 'desc')
                        //         ->limit(10)
                        //         ->pluck('tokenno')   // only fetch tokenno column
                       ...
                       $svcType = Service::select('service_name','id')
                        ->where('type', 1)
    ...
    public function checkweb(Request $request)
    {
        // dd($request) ; 
        ...
        if(!$exists){
            return response()->json(['found' => false, 'message' => 'Code & Webfile match not active!!!']);
        }
    ...
    {
        $RecptNo = $request->input('RecNo');
        $Book = $request->input('Book');
        $data = false ; 
        $message  ='' ;
        $book = MoneyReceipt::where('BookNo', $Book)->first();

        if ($book) {
            if ( $RecptNo < $book->startNo ||  $RecptNo > $book->endNo) {
                $data = false; 
                $message  = "Receipt number must be between {$book->startNo} and {$book->endNo} for BookNo $Book" ; 
            }
            else{
                $recptExist = ForeignPass::select('id')->where('BookNo',$Book)->where('ReceiptNo',$RecptNo)->where('web_ref','!=',$request->recId)->first() ; 
                if($recptExist){
                    $message  = "Receipt number: $RecptNo for BookNo: $Book Already used." ; 
                    $data = false; 
                }
                else{
                    $data = true; 
   ...
   // dd($request->a_name) ; 
        ...
        //     ]
        // ],
        // ['Webfile', 'rejectId'], // Unique constraint keys
        // ['updated_at']); // 
        ...
        return redirect()->route('frpReceive.index')->with('success', 'Webfile Rejected successfully.');
    ...
    public function edit($id)
    {
        ...
        if($data->centerId != $centerId ){
            ...
            $stickers = StickerMap::select('id','sticker')->get() ; 
             // dd($data->visatype, $visaType->pluck('id'));
            return view('pages.AppReceive.partials.edit', compact( 'visaType',  'data','stickers'));
        }
        ...
        if ($apprec) {
            ...
            if($apprec->centerId != $cenId ){
                ...
                if($del){
                    ...
                    if($apprec->codeId){
                         $update = QueueCode::find($apprec->codeId)->update([
                            'status'       =>  1, 
                            'updated_at'=>Date('Y-m-d H:i:s')
                        ]);
                    }
     ...
     */ 
    public function store(StoreForeignReceiveRequest $request): RedirectResponse  
    {    //: RedirectResponse    //StoreAppReceiveRequest
       // dd($request->all());
        try {
            ...
            if($data['gratis']!="1"){
                $book = MoneyReceipt::where('BookNo', $data['BookNo'])->first();
                ...
                if (
                    $data['RecptNo'] < $book->startNo ||  $data['RecptNo'] > $book->endNo
                 ) {
                    return back()->withErrors([
                        'RecptNo' => "Receipt number must be between {$book->startNo} and {$book->endNo} for BookNo {$data['BookNo']}."
                    ]);
                }
      // dd('ok') ; 
            ...
            if(substr($data['contact'], 0, 1)!='0'){
                 return redirect()->route('frpReceive.index')
            ...
            if (!$queueId) {
                return redirect()->route('frpReceive.index')
                    ->with('error', "Code & Webfile match not active!!!");
            }
            ...
            if($msg !='' ){
                 return redirect()->route('frpReceive.index')->with('error', $msg);
            }
            else{
                $tdd = '';
              ...
              $record = AppReceive::updateOrCreate(
                ...
                if($id){
                    ...
                    if($save){
                        ...
                        $fpsave = ForeignPass::updateOrCreate(
                            [ 'web_ref' => $id  ], // Matching condition
                            [
                                'centerId'     => $cenId,
                                'Date'         => date('Y-m-d'),
                                'gratis'=> $data['gratis'],
                                'BookNo'     =>$data['BookNo'],
                                'ReceiptNo'  => $data['RecptNo'],
                                'nationality'    => $data['nationality'],
                                'remarks'       => 1,
                                'duration'   => $data['duration'],
                                'entryType'  => $data['entryType'],
                                'visa_fee'      => $data['visafee'],
                                'fax_trans_charge'     => $data['faxcharge'],
                                'icwf'      => $data['icwf'],
                                'visa_app_charge'          => $data['visaApp'],
                                'total_amount'      => $data['totalfee'],
                                'rupee_rate'        => $data['rupee_rate'],
                                'total_rupee'        => $data['totalfee']/$data['rupee_rate'] ,
                                'created_by'   => auth()->user()->id,
                            ]
                        );
                    ...
                    $updateT = TokenLog::where('Date', date('Y-m-d'))
                        ->where('centerId', $cenId)
                        ->where('tokenno', $data['TokenNo'])
                        ->where('token_svc_no',  $queueId->svc)
                        ->update([
                            'ststop'     => now(),
                        ...
                        // dd($removeQ) ; 
                         ...
                         return redirect()->route('frpReceive.print', ['id' => $id]); 

                      }
                }
                else{
                     return redirect()->route('frpReceive.index')->with('error', "Failed to save data");
                }
            ...
            $mess = $e->getMessage(); 
            // dd($mess) ; 
           return redirect()->route('frpReceive.index')->with('error', 'Insert failed '.$mess);
       ...
       if($data->centerId != $cenId){
           ...
           if($data->remarks=="Manual Entry"){
               ...
               $codA = QueueCode::select('random')
                ...
                $fdata = ForeignPass::where('web_ref', $id)->first() ; 
                // dd($fdata) ; 
                return view('pages.FrpReceive.fp_receipt_print', [
                    'datas' =>  $data,
                    'fdata' =>  $fdata,
