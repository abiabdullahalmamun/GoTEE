...
class  Accept4mHCIController extends Controller
{
    /**
    ...
    public function index(Request $request): View
    {
        ...
        try {
            // dd($cenId) ; 
            $cenId = auth()->user()->centerId ;  //added this... 
            $user = auth()->user()->id ; 
            if($cenId){
                $count = AppLog::where('Date', date('Y-m-d'))->where('created_by', $user )->where('stepId',31)->count() ; 

                // dd($count) ; 
              return view('pages.Accept4mHci.index', [
                    'count' => $count,
