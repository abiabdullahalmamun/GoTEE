...
use App\Exports\DeliveryPrintExport;
...
class DeliveryReportController extends Controller
{
    ...
    public function index(Request $request): View 
    {
        ...
        // Filters
            if ($request->filled('from_date') && $request->filled('to_date')) {
                ...
                if($role== 5 ||  $role== 1 ){
                    $userIds = AppLog::select('created_by')->whereBetween('Date',[$request->from_date, $request->to_date])->where('stepId',5); 
                }
                else{
                    $userIds = AppLog::select('created_by')->whereBetween('Date',[$request->from_date, $request->to_date])->where('centerId',$cenId)->where('stepId',5); 
                }
                if($userIds){
            ...
            // dd($data) ; 
            return view('reports.delivery_report.index', compact('data', 'centers', 'cenId','role','users'));
    ...
    {
        if ($request->filled('from_date') && $request->filled('to_date')) {
            return Excel::download(new DeliveryPrintExport($request), 'delivery-report.xlsx');
        }
        else{
            return redirect()->route('delivery-report.index')->with('error', 'Please select from & to Date');
        }
    ...
    public function exportPDF(Request $request)
    {
        ...
        if ($request->filled('from_date') && $request->filled('to_date')) {
           ...
           if($role == 5 ||  $role == 1 ){
            ...
            $data = $query->with(['center','user','webref'])->where('stepId',5)->get();

            // dd($data) ; 
            $pdf = Pdf::loadView('reports.pdf.delivery',compact('data','from','to','cnt','usr'))->setPaper('a4', 'landscape');
