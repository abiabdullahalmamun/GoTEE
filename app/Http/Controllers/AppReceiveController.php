 ...
 // dd($svcIds) ; 
    ...
    public function checkwebpay(Request $request)
    {
        ...
        if (!$exists) {
            return response()->json([
                'found' => false,
                'message' => 'Token, Code & Webfile match not active!!!'
            ]);
        }


        $tokenSvcId = $exists->svclogId;
        // $exists = QueueCode::where('random', $code)
        //             ->where('token',$token)
        //             ->where('centerId',$cenId)
        //             ->where('web',$web)
        ...
        $txnamt = '' ; 
         $passApp = '' ; 
        if($opT=="auto"){
            $ddt = NicAptList::select('passport','Name','contact','dob')->where('webfile',$web)->first() ; 
            if($ddt){
                $name = $ddt->Name ; 
        ...
        $record = SslAptList::select('txnId', 'visatype','txn_date','amount','passport' )
        ...
        if ($record) {
            ...
            if($reply){
                ...
                $save = SslAptList::upsert([
                    ...
                    ['prev_date', 'curr_date', 'apt_hr', 'paystatus', 'checked_user','checked_on','txnId','amount','visatype','txn_date','updated_at','status', 'actVisa','passport']); // 
    ...
    {    //: RedirectResponse    //StoreAppReceiveRequest
       // dd($request->all());
        try {
            ...
            if($data['passport1'] != $data['passport3'] && $data['payment_method']==1){
                 return redirect()->route('app-receive.index')
                    ->with('error', "Passport in Appointment not match !!!");
            }
            ...
            if($webExist){
                ...
                if( $data['op_type']=='auto'){
                    if($data['passport1']!=$data['passport2']){
                         $msg =  "NIC Passport Number Does not match !!!" ; 
                    }
                }
            ...
            if($msg !='' ){
              ...
              $record = AppReceive::updateOrCreate(
                ...
                if($id){
                    ...
                    $latest = TokenLogWeb::where('webfile', $web)
                                ->latest()
                                ->first();
                            ...
                            if ($latest) {
                                $latest->update([
                                    'web_save' => DB::raw('NOW()'),
                                    'difference' => DB::raw('TIMESTAMPDIFF(SECOND, web_scan, NOW())')
                                ]);
                            }
