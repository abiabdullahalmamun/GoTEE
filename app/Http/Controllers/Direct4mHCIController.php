...
class  Direct4mHCIController extends Controller
{
    /**
    ...
    public function index(Request $request): View
    {
        ...
        try {
            ...
            $user = auth()->user()->id ; 
            if($cenId){
                $count = AppLog::where('Date', date('Y-m-d'))->where('created_by', $user )->where('stepId',33)->count() ; 
                ...
                $query->select('id', 'Webfile', 'passport', 'ApplicantName', 'contact');
                    ...
                    ->get();
                // dd($count) ; 
              return view('pages.Direct4mHci.index', [
                  'latest' => $latest,    
                    'count' => $count,
