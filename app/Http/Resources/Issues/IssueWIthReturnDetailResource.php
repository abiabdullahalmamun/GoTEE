...
class IssueWIthReturnDetailResource extends JsonResource
{
        ...
        return [
            ...
            'items' => $this->issueItems->map(function($item) {
                ...
                return [
                    ...
                    'typeId' => optional($item->item->itemType)->id,
                    'typeName' => optional($item->item->itemType)->name,
                    'itemCode' => optional($item->item)->item_code,
                    'uomName' => optional($item->item->itemUOM)->name ?? null,
                    'defaultImage' => optional($item->item->firstImage)->image_url ? url(optional($item->item->firstImage)->image_url) : null,
                    ...
                    'totalReturnQty'=>(float) ($item->issueReturnItems->sum('qty') ?? 0),
                    'totalReturnableQty' => (float) bcsub(
                        (string) $item->qty,
                        (string) ($item->issueReturnItems?->sum('qty') ?? 0),
                        2
                    ),                    'warrantyMonth' => $warrantyMonths,
                    'isWarrantyHave' => $isWarrantyHave,
                    'warrantyExpireDate' => $warrantyExpireDate->format('Y-m-d'),
                    'returnSerials' => optional($item->issueReturnItems)
                        ->flatMap(fn ($returnItem) => optional($returnItem->receiveItem)->stockSerials ?? [])
                        ->map(fn ($serial) => [
                        'id' => optional($serial->itemSerial)->id,
