...
class AppReceiveExport implements FromCollection, WithHeadings, WithEvents
{
    protected $request;
    ...
    public function collection()
    {
       ...
       if ($this->request->filled('visa')) {
            $query->where('visatype', $this->request->visa);
        }
        if ($this->request->filled('sticker')) {
            $query->where('stickertype', $this->request->sticker);
        }

        return $query->with(['center', 'user','visa','sticker'])->get()->values()->map(function ($row, $index) {
             $badges = [
    ...
    {
        return [
           'SL', 'Center', 'Date', 'Webfile', 'Name', 'Passport','StickerType', 'StickerNo','Contact', 'VisaType', 'paytype', 'txn', 'Payment Info /Remarks','Correction Fee','PassQty','Counter','Token' ,'Received By', 'Received At'
        ];
    }
  ...
  public function registerEvents(): array
{
    return [
        AfterSheet::class => function (AfterSheet $event) {
            ...
            $event->sheet->setCellValue('A1', 'Application Received Report');
