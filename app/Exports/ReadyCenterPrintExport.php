...
class ReadyCenterPrintExport implements FromCollection, WithHeadings, WithEvents
{
    protected $request;
    ...
    public function collection()
    {
        ...
        return $query->with(['center', 'webref','user'])->where('stepId',4)->get()->map(function ($row, $index) {
             return [
                 $index + 1,
    ...
    {
        return [
           'SL','Center', 'Date', 'Webfile', 'Name','passport', 'Contact','VisaType','SticketType','StickerNo', 'CreatedBy', 'CreatedAt'
        ];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->setCellValue('A1', 'Ready At Center Report');
