...
@section('content')
    <div class="mx-auto">
        <x-page-header title="Correction Type" />
                    ...
                    <table class="w-full border border-gray-200 text-sm">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="border px-4 py-2 text-left">Type Name</th>
                        ...
                        <tbody id="datatable">
                            @forelse ($query as $key => $data)
                                <tr
                                    ...
                                    <td class="border px-4 py-2">{{ $data->type_name }}</td>
                                    <td class="border px-4 py-2">{{ $data->created_by }}</td>
                              ...
                              <!--           <button onclick="viewFloor({{ $data }})"
                                        ...
                                        <form action="{{ route('shops.destroy', $data->id) }}" method="POST"
    ...
    <div id="ShopModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-[1100]">
        <div class="bg-white p-6 rounded w-96 relative">
            <h2 class="text-lg font-bold mb-4" id="modalTitle">Create Correction Type</h2>
            @include('pages.correction_types.partials.create')
        </div>
    </div>
    ...
    <div id="editModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-[1100]">
        <div class="bg-white p-6 rounded w-96">
            <h2 class="text-lg font-bold mb-4">Edit Correction Type</h2>
            @include('pages.correction_types.partials.edit')
        </div>
    </div>
    ...
    <div id="viewModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-[1100]">
        <div class="bg-white p-6 rounded w-96 relative">
            <h2 class="text-lg font-bold mb-4">View Correction Type</h2>
            @include('pages.correction_types.partials.view')
        </div>
    </div>
@endsection

@push('scripts')
    @vite(['resources/js/scripts/shop.js'])
