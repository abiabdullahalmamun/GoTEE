...
@section('content')
                        ...
                        <tbody id="datatable">
                            @forelse ($query as $key => $data)
                                <tr
                                    ...
                                    <td class="border px-4 py-2">{{ $data->ShopName }}</td>
                                    <td class="border px-4 py-2">{{ $data->Contact }}</td>
                                    <td class="border px-4 py-2">
                                        {{ \Illuminate\Support\Str::limit($data->message, 100, '...') }}</td>
    ...
    <div id="ShopModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-[1100]">
        <div class="bg-white p-6 rounded w-96 relative">
            <h2 class="text-lg font-bold mb-4" id="modalTitle">Create Shop</h2>
            @include('pages.shops.partials.create')
        </div>
    </div>
    ...
    <div id="editModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-[1100]">
        <div class="bg-white p-6 rounded w-96">
            <h2 class="text-lg font-bold mb-4">Edit Shop</h2>
            @include('pages.shops.partials.edit')
        </div>
    </div>
    ...
    <div id="viewModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-[1100]">
        <div class="bg-white p-6 rounded w-96 relative">
            <h2 class="text-lg font-bold mb-4">View Floor</h2>
            @include('pages.shops.partials.view')
        </div>
    </div>
@endsection
