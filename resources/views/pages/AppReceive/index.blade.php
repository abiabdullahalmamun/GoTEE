    ...
    <div class="mx-auto">
		...
		<div class="status bg-white shadow rounded mt-2 max-w-full mx-auto">
            ...
            <table width=100%>
			  <tr>
			  	<td width="22%">
    <label class="label" for="sticker_type">Service</label>

    <div class="flex items-center gap-3">
        <!-- Service Select -->
        <select class="form-control px-3 py-2 border"
                name="svctypeId"
                required
                id="svctypeId">
            ...
            @foreach($svcIds as $item)
                @continue($item['service_name'] == 'FOREIGN')
                <option value="{{ $item['id'] }}" data-type="{{ $item['type'] }}"
                        {{ $lastSvc == $item['id'] ? 'selected' : '' }}>
                    {{ $item['service_name'] }}
                </option>
            @endforeach
        </select>
        ...
        <div class="flex items-center gap-1 whitespace-nowrap">
            ...
            <input type="text"
                   id="PendingQty"
                   name="PendingQty"
                   class="number px-2 py-1 w-16 text-left"
                   readonly
                   value="">
        </div>
    </div>

    <input type="text" id="svcType" name="svcType" hidden value="{{ $svcType }}">
</td>
	<!-- 			<td width="22%">
					<label class="label" for="sticker_type">Service</label>
					<br>
					<select class="form-control w-full px-3 py-2 border"
					        name="svctypeId"
					        required
					        id="svctypeId">
					...
					<div class="number">
						<b><input type="text" id="PendingQty" name="PendingQty" class="number px-2 py-1" readonly  value="1"></b>
					</div>
				...
				<td width="22%">
					...
					<div class="number">
						<b><input type="text" id="counterNo" name="counterNo" class="number px-2 py-1" readonly  value="{{$counter}}"></b>
					<!-- </select> -->
			  ...
			  <tr>
				<td width=70px;>
					<div class="currentq">
						<button type="button"
						    id="loadTokenNo"
						    class="callbtn bg-blue-500 text-white px-3 py-1 max-w-full">
						    <b>Call</b>
						</button>
						<input type="text" id="CallEnable" name="CallEnable" readonly hidden class="number px-2 py-1" value="{{$enCall}}">
					
						<div class="label">Recall Q</div>
						
						<ol id="currQList" class="QList">
							@foreach ($recallQ as $item)
...
<div class="form-container">
	<div id="reason_check" style="display: none">
		<div class="form-row">
			<label for="form_date">Comment </label>
			<input class="barcode" type="text" value="" id="comment" name="comment" >
		</div>
	</div>
	<div class="form-row">
	...
	<div id="security_code" style="display: none">
	  ...
	  <div id="web_check" style="display: none"> 
		...
		<!-- 	<input class="typeotp" type="text" id="otp" name="otp" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');" maxlength="4" class="hideDiv" title="OTP type here" placeholder="OTP"> -->
			<input class="pass3" type='text' id='passport3' name='passport3'  maxlength="10"   readonly > 
		</div>
	...
	<div id="fee_check" style="display: none">
		...
		<div class="form-row">
			...
			<input class="dob" type="text" id="dob" name="dob" readonly >
			<input class="age" type="text" id="age" name="age" readonly >
		</div>
		<div class="form-row2">
			<label>BIO: </label>
		...
		<button id="bt3" type="button" class="reject text-white px-4 py-2 rounded"> RETURN </button>
