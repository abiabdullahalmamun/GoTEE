<form method="POST" id="createShopForm" action="{{ route('visa_types.store') }}">
    @csrf
    @method('POST')
    <div class="row">
        <div class="mb-3 col-12 col-md-4 col-lg-6">
            <div class="form-group">
                <label for="name">Visa Type</label><br><input type="text" name="visa_type"
        ...
        <div class="mb-3 col-12 col-md-4 col-lg-6 flex space-x-4">
            <div class="form-group flex-1">
              ...
              <input type="text" name="symbol" class="form-control @error('symbol') is-invalid @enderror w-full px-3 py-2 border rounded" value="{{ old('symbol') }}" placeholder="type" required>
                @error('mac')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group flex-1">
                ...
                <input type="number" name="days" class="form-control @error('days') is-invalid @enderror w-full px-3 py-2 border rounded" value="{{ old('days') }}" placeholder="" required>
                @error('days')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
