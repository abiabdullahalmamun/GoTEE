...
@section('content')
    <div class="mx-auto">
        <x-page-header title="Visa Duration" />
                        ...
                        <tbody>
                            @forelse ($query as $key => $data)
                                ...
                                <!--         <button onclick="editDevice({{$data}})"
                                        ...
                                        <form action="{{ route('visaDuration.destroy', $data->id) }}" method="POST"
    ...
    <div id="ShopModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-[1100]">
        <div class="bg-white p-6 rounded w-96 relative">
            <h2 class="text-lg font-bold mb-4" id="modalTitle">Create Visa Duration</h2>
            @include('pages.visaDuration.partials.create')
        </div>
    </div>
    ...
    <div id="editModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-[1100]">
        <div class="bg-white p-6 rounded w-96">
            <h2 class="text-lg font-bold mb-4">Edit Visa Duration</h2>
            @include('pages.visaDuration.partials.edit')
        </div>
    </div>
...
@push('scripts')
    @vite(['resources/js/scripts/visaDuration.js'])
@endpush
