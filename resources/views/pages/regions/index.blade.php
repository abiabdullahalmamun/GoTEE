...
@section('content')
                    ...
                    <table class="w-full border border-gray-200 text-sm">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="border px-4 py-2 text-left">Region Text</th>
                        ...
                        <tbody id="datatable">
                            @forelse ($query as $key => $data)
                                <tr
                                    ...
                                    <td class="border px-4 py-2">{{ $data->region_name }}</td>
                                    <td class="border px-4 py-2">{{ $data->region_text }}</td>
    ...
    <!-- Create Menu Modal -->
    <div id="RegionModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-[1100]">
        <div class="bg-white p-6 rounded w-96 relative">
            <h2 class="text-lg font-bold mb-4" id="modalTitle">Create Region</h2>
            @include('pages.regions.partials.create')
        </div>
    </div>
    ...
    <div id="editModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-[1100]">
        <div class="bg-white p-6 rounded w-96">
            <h2 class="text-lg font-bold mb-4">Edit Region</h2>
            @include('pages.regions.partials.edit')
        </div>
    </div>
    ...
    <div id="viewModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-[1100]">
        <div class="bg-white p-6 rounded w-96 relative">
            ...
            @include('pages.regions.partials.view')
        </div>
    </div>
@endsection
