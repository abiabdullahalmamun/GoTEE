...
@section('content')
    <div class="mx-auto">
        <x-page-header title="Send To MOFA" />
        @if (session('success'))
    ...
    <div class="flex justify-between items-center mb-4 w-full">
        ...
        <div class="flex items-center space-x-2">
            ...
            <input type="text" id="web" name="web"
                   class="form-control px-3 py-2 border rounded w-64">
        </div>

        <!-- Second Input -->
        <div class="flex items-center space-x-2">
            <label for="remarks" class="text-blue-700"><i></i></label>
            <input type="text" id="remarks" name="remarks"  class="px-3 py-2 border rounded w-64 bg-red-500 font-bold text-yellow-300"
                    value="Sent to MOFA" readonly>   
                    <!-- class="form-control px-3 py-2 border rounded w-64"  -->
        </div>

        <!-- Label on Right -->
        <div>
            <label for="total" class="text-gray-700"><i>Total Send2MOFA:</i></label>
...
@push('scripts')
    @vite(['resources/js/scripts/send2MOFA.js'])
