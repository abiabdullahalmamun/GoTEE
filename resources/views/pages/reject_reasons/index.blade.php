...
@section('content')
    <div class="mx-auto">
        <x-page-header title="Reject Reasons" />
        @if (session('success'))
        ...
        <div class="bg-white shadow rounded p-6 mt-1 mx-auto">
            ...
            <div class="mt-4">
                <div id="parent-floor-list" class="floor-list overflow-y-hidden overflow-x-auto ">
                    <table id="recordTable" class="w-full border border-gray-200 text-sm">
                        <thead class="bg-gray-100">
                            <tr>
                                <th class="border px-4 py-2 text-left">SL</th>
                                <th class="border px-4 py-2 text-left">Reason Name</th>
                        ...
                        <tbody>
                            @forelse ($query as $key => $data)
                                <tr
                                    ...
                                    <td class="border px-4 py-2">{{ $data->reason_name }}</td>
                              ...
                              <!--           <button onclick="viewFloor({{ $data }})"
                                        ...
                                        <form action="{{ route('reject_reasons.destroy', $data->id) }}" method="POST"
    ...
    <div id="ShopModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-[1100]">
        <div class="bg-white p-6 rounded w-96 relative">
            <h2 class="text-lg font-bold mb-4" id="modalTitle">Create Reject Reasons</h2>
            @include('pages.reject_reasons.partials.create')
        </div>
    </div>
    ...
    <div id="editModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-[1100]">
        <div class="bg-white p-6 rounded w-96">
            <h2 class="text-lg font-bold mb-4">Edit Reject Reasons</h2>
            @include('pages.reject_reasons.partials.edit')
        </div>
    </div>
    ...
    <div id="viewModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-[1100]">
        <div class="bg-white p-6 rounded w-96 relative">
            <h2 class="text-lg font-bold mb-4">View Reject Reasons</h2>
            @include('pages.reject_reasons.partials.view')
        </div>
    </div>
@endsection

@push('scripts')
    @vite(['resources/js/scripts/reject.js'])
