<form method="POST" id="createReasonForm" action="{{ route('reject_reasons.store') }}">
    @csrf
    @method('POST')
    <div class="row">
        <div class="mb-3 col-12 col-md-4 col-lg-6">
            <div class="form-group">
                <label for="name">Reason</label><br><input type="text" name="reasonName"
                    class="form-control @error('name') is-invalid @enderror w-full px-3 py-2 border rounded"
                    value="{{ old('name') }}" placeholder="Enter Reason..." required>
                @error('reasonName')
