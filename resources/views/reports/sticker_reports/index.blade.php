...
@section('content')
<div class="mx-auto">
    <x-page-header title="Barcode Sticker Print Report" />
       @if (session('success'))
	...
	<div class="bg-white shadow rounded p-2 mt-1 mx-auto">
		<div class="appReceive">
			<form method="GET" action="{{ route('sticker-Report.index') }}" class="mb-2 flex gap-2">
				From:
		...
		<div class=" flex gap-2">
			<a href="{{ route('sticker-Report.excel', request()->all()) }}" class="bg-green-600 text-white px-2 py-1 rounded text-sm">Excel</a>
			<a href="{{ route('sticker-Report.pdf', request()->all()) }}" class="bg-red-600 text-white px-2 py-1 rounded text-sm">PDF</a>
