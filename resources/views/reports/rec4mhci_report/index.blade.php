...
@section('content')
<div class="mx-auto">
    <x-page-header title="Passport Sent to IVAC Report" />
       @if (session('success'))
	...
	<div class="bg-white shadow rounded p-2 mt-1 mx-auto">
		<div class="appReceive">
			<form method="GET" action="{{ route('rec4mhci-report.index') }}" class="mb-2 flex gap-2">
				From:
...
@endif
		...
		<div class=" flex gap-2">
			<a href="{{ route('rec4mhci-report.excel', request()->all()) }}" class="bg-green-600 text-white px-2 py-1 rounded text-sm">Excel</a>
			<a href="{{ route('rec4mhci-report.pdf', request()->all()) }}" class="bg-red-600 text-white px-2 py-1 rounded text-sm">PDF</a>
		...
		<div class="overflow-x-auto">
			<table id="reportTable" class="table-auto w-full">
				...
				<tbody>
					...
					<td class="text-center">{{ $row->webref->Date ?? '' }}</td>
					<td>{{ $row->user->name ?? ''}}</td>
