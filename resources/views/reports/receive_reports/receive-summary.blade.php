...
@section('content')
<div class="mx-auto">
	<x-page-header title="Receive Summary" />
	@if (session('success'))
                ...
                <table  id="reportTable" class="w-full text-sm border border-gray-300">
                   ...
                   @foreach($results as $index => $row)
                        <tr>
                            ...
                            <td class="text-center align-middle border px-2 py-1">
                                 @if($report === 'VisaType Summary')
                                    {{ $row->visa->visa_type ?? 'N/A' }}
                                @else
                                    {{ $row->sticker->sticker ?? 'N/A' }}
                                @endif

                            </td>
                            <td class="border px-2 py-1 text-center align-middle">{{ $row['total'] }}</td>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td class="text-center align-middle border px-2 py-1"><b> {{$grandTotal}} </b></td>
                        </tr>
                    </tfoot>
